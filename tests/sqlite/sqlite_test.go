...
func setupDatabase(ts *TestDBSuite) {
	...
	_, err = ts.db.DBX.ExecContext(ctxDefault, query)
	if err != nil {
		ts.FailNowf("unable to create table ", "[user] %s", err.Error())
	}
	ts.T().Log("Таблица [user] создана\n")
}
...
func (ts *TestDBSuite) TestInsertBatch() {

	insUsers := []*User{
		{Name: "Иванов", Age: 26},
		{Name: "Петров", Age: 40},
	}
	ts.Suite.Run("batch insert Test", func() {
		query := `INSERT INTO user (name, age) VALUES (:name, :age)`
		_, err := ts.db.NamedExecContext(ctxDefault, query, insUsers)
		ts.NoError(err)
	})

	query := `select name, age from user`
	gotMap, err := ts.db.SelectMapsContext(ctxDefault, query)
	ts.NoError(err)
	ts.Len(gotMap, 2)
	ts.Equal(int64(40), gotMap[1]["age"])
	//ts.T().Logf("получим в map: %+v", gotMap)
}
